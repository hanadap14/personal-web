                ...
                <div class="table-responsive">
                    <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                        ...
                        <tbody>
                            ...
                            <tr class="odd">
                                <td><img src="{{ url('assets/portfolio/'.$port->photo) }}" alt="" width="150px"></td>
                                <td>{{ $port->title }}</td>
                                <td>{{ $port->year }}</td>
                                <td>{{ $port->created_at }}</td>
                                <td>{{ $port->updated_at }}</td>
                                <td>
                                    <a href="{{ url('admin/portfolio/'.$port->id.'/edit') }}" class="btn btn-warning mb-2"><i class="fas fa fa-pen"></i></a>
                                    <form action="{{ url('admin/portfolio/'.$port->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger mb-2" onclick="return confirm('Are you sure want to delete {{ $port->title }}')"><i class="fas fa fa-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
